# Настройка электронных чеков на Email

## Принцип работы

1. Гость выбирает получить чек на Email.

3. Приложение передаёт в IIKO данные: "печатный чек не нужен" и Email гостя.

5. Фискальный регистратор отправляет гостю электронный чек через оператор фискальных данных (ОФД).

> Чек отправляет именно фискальный регистратор, а не IIKO или приложение. Настройка в IIKO проводится для того, чтобы гостю был доступен выбор способа получения чека.
...
Если гость выбирает печатный чек, то приложение передаёт в IIKO параметр: 
